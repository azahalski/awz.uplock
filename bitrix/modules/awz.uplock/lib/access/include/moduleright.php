...
use Awz\Uplock\Access\Custom\ComponentConfig;
                ...
                window.awz_right_dialog = {
                    ...
                    reloadItems: function(items, event){
                        ...
                        items.forEach(function(itm){
                            if(itm.getEntityId()==='user' || itm.getEntityType()==='awzuplock-user'){
                                let itmEv = {
                                    type: 'users',
                                    id: 'U'+itm.getId(),
                                    key: 'U'+itm.getId(),
                            ...
                            }else if(itm.getEntityType()==='awzuplock-group'){
                                let itmEv = {
                                    type: 'groups',
                                    id: 'G'+itm.getId(),
                                    key: 'G'+itm.getId(),
                    ...
                    load: function(groupKey, options){
                        ...
                        if(groupData.hasOwnProperty('members')){
                            for(k in groupData['members']){
                                ...
                                if(itm.type==='users'){
                                    preselectedItems.push(['awzuplock-user',itm.id]);
                                }
                                else if(itm.type==='awzuplock-user'){
                                    preselectedItems.push(['awzuplock-user',itm.id]);
                                }
                                else if(itm.type==='awzuplock-group'){
                                    preselectedItems.push(['awzuplock-group',itm.id]);
                                }
